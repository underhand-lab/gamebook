...
MVP API는 Home Feed, Timeline, Match Detail, Write Review, Profile, Statistics를 우선 지원한다. Stadium Collection, Badges, Lists, Wrapped는 Post-MVP 화면으로 분리하되 읽기용 placeholder API는 확장 영역으로 남긴다.

Timeline은 MVP에서 Profile과 함께 제공되는 개인 기록 중심 화면이다. Diary는 비공개 기록 정책이 확정된 뒤 Post-MVP에서 제공한다.
