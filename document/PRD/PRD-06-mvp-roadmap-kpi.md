...
- 사용자는 태그를 검색하고 태그 결과 페이지로 이동할 수 있다.
- 사용자는 일자별 경기 목록에서 응원팀, 종목, 리그 필터를 사용할 수 있다.

### Match Logging
- 사용자는 경기 관람 기록을 저장할 수 있다.
- 관람 방식과 응원팀 또는 팬 관점을 저장할 수 있다.
- 저장된 관람 기록은 Timeline에 MatchLogAggregate로 표시된다.
- 저장된 관람 기록은 Calendar / Diary에 날짜 단위로 표시된다.

### Review / Rating
- 사용자는 별점과 리뷰를 작성할 수 있다.
- 사용자는 스포일러 여부와 태그를 입력할 수 있다.
- 경기 페이지는 평균 평점과 별점 분포를 함께 보여준다.
...
- 프로필은 총 경기 수, 직관 수, 라이브 수, 평균 평점, 별점 분포, 리뷰 수, 좋아요 수, 응원팀을 보여준다.
- 사용자는 프로필에서 리그별 여러 응원팀을 선택할 수 있다.
...
- Calendar / Diary에서 날짜별 내 기록을 볼 수 있다.
...
- `tag_result_viewed`
- `date_matches_viewed`
- `date_match_filter_used`
- `calendar_viewed`
- `diary_date_selected`
- `favorite_teams_updated`
- `review_liked`
- `mvp_vote_submitted`
